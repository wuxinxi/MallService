...
class ExpiringTokenAuthentication(BaseAuthentication):
    ...
    def authenticate_credentials(self, key):
        ...
        # token有效期:3天
        if (datetime.datetime.now() - token.created) > datetime.timedelta(days=3):
            raise exceptions.AuthenticationFailed('请重新登录')

        # 加入缓存增加查询速度，下面和上面是配套的，上面没有从缓存中读取，这里就不用保存到缓存中了
        if token:
            token_cache = 'token_' + key
            cache.set(token_cache, token.user, 24 * 3 * 60 * 60)
